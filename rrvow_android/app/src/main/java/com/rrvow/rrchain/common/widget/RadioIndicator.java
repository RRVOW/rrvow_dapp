...
public class RadioIndicator extends android.support.v7.widget.AppCompatRadioButton {
    ...
    protected void onDraw(Canvas canvas) {
        if (isChecked()) {
            ...
            //canvas.drawLine((getMeasuredWidth() - tw) / 2, getMeasuredHeight() - stroke,
            // (getMeasuredWidth() + tw) / 2, getMeasuredHeight() - stroke, paint);
            canvas.drawLine(0, getMeasuredHeight() - stroke, getMeasuredWidth(),
                    getMeasuredHeight() - stroke, paint);
        }
        super.onDraw(canvas);
    }
}
