...
import com.rrvow.rrchain.common.utils.ColorUtils;
...
public class SwitchButton extends CompoundButton {
    ...
    private static int[] CHECKED_PRESSED_STATE = new int[]{android.R.attr.state_checked, android
            .R.attr.state_enabled, android.R.attr.state_pressed};
    ...
    private void init(AttributeSet attrs) {
        ...
        mClickTimeout = ViewConfiguration.getPressedStateDuration() + ViewConfiguration
                .getTapTimeout();
        ...
        TypedArray ta = attrs == null ? null : getContext().obtainStyledAttributes(attrs, R
                .styleable.SwitchButton);
        if (ta != null) {
            ...
            thumbRadius = ta.getDimension(R.styleable.SwitchButton_kswThumbRadius, Math.min
                    (thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.SwitchButton_kswBackRadius, thumbRadius +
                    density * 2f);
            ...
            backMeasureRatio = ta.getFloat(R.styleable.SwitchButton_kswBackMeasureRatio,
                    backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.SwitchButton_kswAnimationDuration,
                    animationDuration);
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        int minHeight = ceil(Math.max(mThumbSizeF.y, mThumbSizeF.y + mThumbMargin.top +
                mThumbMargin.right));
    ...
    private void setup() {
        ...
        float textDiffWidth = Math.min(0, (Math.max(mThumbSizeF.x * mBackMeasureRatio,
                mThumbSizeF.x + mTextWidth) - mThumbRectF.width() - mTextWidth) / 2);
        ...
        mBackRectF.set(backLeft + textDiffWidth,
                ...
                backLeft + mThumbMargin.left + Math.max(mThumbSizeF.x * mBackMeasureRatio,
                        mThumbSizeF.x + mTextWidth) + mThumbMargin.right - textDiffWidth,
                mThumbRectF.bottom + mThumbMargin.bottom - textDiffHeight);

        mSafeRectF.set(mThumbRectF.left, 0, mBackRectF.right - mThumbMargin.right - mThumbRectF
                .width(), 0);
        ...
        if (mBackDrawable != null) {
            mBackDrawable.setBounds((int) mBackRectF.left, (int) mBackRectF.top, ceil(mBackRectF
                    .right), ceil(mBackRectF.bottom));
        }

        if (mOnLayout != null) {
            float marginOnX = mBackRectF.left + (mBackRectF.width() - mThumbRectF.width() -
                    mOnLayout.getWidth()) / 2 - mThumbMargin.left + mTextMarginH * (mThumbMargin
                    .left > 0 ? 1 : -1);
        ...
        if (mOffLayout != null) {
            float marginOffX = mBackRectF.right - (mBackRectF.width() - mThumbRectF.width() -
                    mOffLayout.getWidth()) / 2 + mThumbMargin.right - mOffLayout.getWidth() -
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (switchText != null && textRectF != null) {
            int alpha = (int) (255 * (getProcess() >= 0.75 ? getProcess() * 4 - 3 : (getProcess()
                    < 0.25 ? 1 - getProcess() * 4 : 0)));
            int textColor = getProcess() > 0.5 ? mOnTextColor : mOffTextColor;
            int colorAlpha = Color.alpha(textColor);
            colorAlpha = colorAlpha * alpha / 255;
            switchText.getPaint().setARGB(colorAlpha, Color.red(textColor), Color.green
                    (textColor), Color.blue(textColor));
            canvas.save();
    ...
    public void setThumbMargin(RectF thumbMargin) {
        if (thumbMargin == null) {
            ...
            setThumbMargin(thumbMargin.left, thumbMargin.top, thumbMargin.right, thumbMargin
                    .bottom);
        }
    }
