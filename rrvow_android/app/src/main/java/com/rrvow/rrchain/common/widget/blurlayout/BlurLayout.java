package com.rrvow.rrchain.common.widget.blurlayout;
...
/**
 * A {@link ViewGroup} that blurs all content behind it. Automatically creates bitmap of parent
 * content
 * and finds its relative position to the top parent to draw properly regardless of where the
 * layout is
 * placed.
 */
public class BlurLayout extends FrameLayout {
    ...
    /**
     * Reference to View for top-parent. For retrieval see
     * {@link #getActivityView() getActivityView}.
     */
    ...
    public BlurLayout(Context context, AttributeSet attrs) {
        ...
        try {
            mDownscaleFactor = a.getFloat(R.styleable.BlurLayout_blk_downscaleFactor,
            ...
            mCornerRadius = a.getDimension(R.styleable.BlurLayout_blk_cornerRadius,
    ...
    private Bitmap blur() {
        ...
        rightOffset = x + getWidth() + rightOffset <= screenWidth ? rightOffset : screenWidth -
                getWidth() - x;
            ...
            try {
                ...
                bitmap = getDownscaledBitmapForView(
                        ...
                        new Rect(
                                ...
                                pointRelativeToActivityView.x + getWidth() + Math.abs(leftOffset)
                                        + rightOffset,
                                pointRelativeToActivityView.y + getHeight() + Math.abs(topOffset)
                                        + bottomOffset
                        ),
                        mDownscaleFactor
                );
    ...
    /**
     * Casts context to Activity and attempts to create a view reference using the window decor
     * view.
     *
    ...
    private Bitmap getDownscaledBitmapForView(View view, Rect crop, float downscaleFactor) throws
        ...
        if (screenView.getWidth() <= 0 || screenView.getHeight() <= 0 || width <= 0 || height <=
                0) {
    ...
    public void lockView() {
        ...
        if (mActivityView != null && mActivityView.get() != null) {
            ...
            try {
                setAlpha(0f);
                mLockedBitmap = getDownscaledBitmapForView(view, new Rect(0, 0, view.getWidth(),
                        view.getHeight()), mDownscaleFactor);
                setAlpha(1f);
